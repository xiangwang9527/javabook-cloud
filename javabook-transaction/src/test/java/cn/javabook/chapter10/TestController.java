...
public class TestController extends Thread {
	...
	@Test
	public void test() {
		Object object = paymentOrderService.getSavedObject("111");
		...
		public void run() {
			...
			for (int j = 0; j < 10000; j++) {
				try {
					...
					// 封装事务消息
					TransactionMessage message = paymentOrderService.balancePay(taskid, userid, 0, 1, "余额支付压力测试任务");
					// TODO：省略中间若干步骤
					// 发送消息
					transactionMessageService.saveAndSendMessage(message);
				} catch (Exception e) {
